...
--bc1 fix for OCC / raze holy city
--TODO: g&k compatibility
-------------------------------------------------
-- Global Constants
-------------------------------------------------
local g_Tabs = {
...
local g_WorldReligionSortOptions = {
	{
...
-- Global Variables
-------------------------------------------------
local g_ReligiousBeliefsManager = StackInstanceManager( "YourReligiousBeliefInstance", "Base", Controls.YourReligiousBeliefsStack )
local g_FaithModifierManager = StackInstanceManager( "FaithModifierInstance", "FaithModifier", Controls.FaithModifierStack )
local g_WorldReligionsManager = StackInstanceManager( "WorldReligionInstance", "Base", Controls.WorldReligionsStack )
local g_BeliefsManager = StackInstanceManager( "BeliefInstance", "Base", Controls.BeliefsStack )
local g_BeliefSortFunction, g_WorldReligionSortFunction, g_CurrentTab, g_PopupInfo
...
AddSerialEventGameMessagePopup( function( popupInfo )
	if popupInfo.Type ~= ButtonPopupTypes.BUTTONPOPUP_RELIGION_OVERVIEW or Game.IsOption(GameOptionTypes.GAMEOPTION_NO_RELIGION) then
		return
	end
...
end, ButtonPopupTypes.BUTTONPOPUP_RELIGION_OVERVIEW )

-------------------------------------------------------------------------------
-------------------------------------------------------------------------------
function IgnoreLeftClick()
	-- just swallow it
end
-------------------------------------------------------------------------------
-------------------------------------------------------------------------------
...
-- TODO g&k	
	...
	if #unlockedGreatPeople == 0 then
		local c = Controls.GreatPersonName1
	...
	if player:HasCreatedReligion() then
		...
		for i,v in ipairs(Game.GetBeliefsInReligion(eReligion)) do
			...
			if belief then
				...
				entry.Base:SetSizeY( entry.BeliefDescription:GetSizeY() + 18 );
			end
		end
	elseif player:HasCreatedPantheon() then
...
--bc1 fix for OCC / raze holy city
				local holyCityName = "TXT_KEY_RO_WR_UNKNOWN_HOLY_CITY";
