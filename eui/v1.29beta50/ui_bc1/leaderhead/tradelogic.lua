...
if not IsCiv5 then --no glass panel in civBE
	Controls.UsGlass = Controls.UsPanel
	Controls.ThemGlass = Controls.ThemPanel
end

local g_UsTableCitiesIM		= StackInstanceManager( "CityInstance", "Button", Controls.UsTableCitiesStack )
local g_UsPocketCitiesIM	= StackInstanceManager( "CityInstance", "Button", Controls.UsPocketCitiesStack )
local g_ThemTableCitiesIM	= StackInstanceManager( "CityInstance", "Button", Controls.ThemTableCitiesStack )
local g_ThemPocketCitiesIM	= StackInstanceManager( "CityInstance", "Button", Controls.ThemPocketCitiesStack )
local g_UsTableVoteIM, g_UsPocketVoteIM, g_ThemTableVoteIM, g_ThemPocketVoteIM
if IsCiv5BNW then
	g_UsTableVoteIM		= StackInstanceManager( "TableVote", "Container", Controls.UsTableVoteStack )
	g_UsPocketVoteIM	= StackInstanceManager( "PocketVote", "Button", Controls.UsPocketVoteStack )
	g_ThemTableVoteIM	= StackInstanceManager( "TableVote", "Container", Controls.ThemTableVoteStack )
	g_ThemPocketVoteIM	= StackInstanceManager( "PocketVote", "Button", Controls.ThemPocketVoteStack )
end
...
local g_iPeaceDuration = IsCiv5notVanilla and Game.GetPeaceDuration() or GameDefines.PEACE_TREATY_LENGTH or 10
...
function DoUpdateButtons( diploMessage )
	...
	elseif g_PVPTrade then -- Dealing with a human in a MP game
		...
		elseif g_PVPTrade == 1 then -- this is our proposal
			...
			propose = propose and not ( IsCiv5BNW and g_pUsTeam:CanChangeWarPeace( g_iThemTeam ) and not g_pUsTeam:IsAtWar( g_iThemTeam ) and g_pUsTeam:CanDeclareWar( g_iThemTeam ) )
			Controls.CancelButton:SetHide( true )
			Controls.ModifyButton:SetHide( true )
...
--	Controls.ThemPocketTradeAgreement:SetHide( false ) --Trade agreement disabled for now
	...
	local strRAString = L( "TXT_KEY_DIPLO_RESCH_AGREEMENT_US", Game.GetResearchAgreementCost( g_iUs, g_iThem ) )
	...
	elseif g_PVPTrade then
		...
		TruncateString(Controls.ThemName, Controls.ThemTablePanel:GetSizeX() - Controls.ThemTablePanel:GetOffsetX(),
							L( "TXT_KEY_DIPLO_ITEMS_LABEL", g_pThem:GetNickName() ))
		Controls.ThemCiv:SetText( "(" .. GameInfo.Civilizations[ g_pThem:GetCivilizationType() ]._Name .. ")" )
		...
		Controls.LeaderNameItems:SetText(L("TXT_KEY_DIPLO_ITEMS_LABEL",L(g_pThem:GetNameKey())))
...
--		local themCivInfo = GameInfo.Civilizations[ g_pThem:GetCivilizationType() ]
	...
	SetEnabledAndTextAndToolTip( Controls.UsPocketGold, isEnabled,
								g_Deal:GetGoldAvailable( g_iUs, -1 ) .. " " .. L"TXT_KEY_DIPLO_GOLD", -- This is -1 because we're not changing anything right now
	...
	SetEnabledAndTextAndToolTip( Controls.ThemPocketGold, isEnabled,
								g_Deal:GetGoldAvailable( g_iThem, -1 ) .. " " .. L"TXT_KEY_DIPLO_GOLD", -- This is -1 because we're not changing anything right now
								IsCiv5BNW and not isEnabled and "[COLOR_WARNING_TEXT]"..L"TXT_KEY_DIPLO_NEED_DOF_TT_ONE_LINE".."[ENDCOLOR]" )
	...
	SetEnabledAndText( Controls.UsPocketGoldPerTurn,
						...
						g_pUs:CalculateGoldRate() .. " " .. L"TXT_KEY_DIPLO_GOLD_PER_TURN" )
	...
	SetEnabledAndText( Controls.ThemPocketGoldPerTurn,
						...
						g_pThem:CalculateGoldRate() .. " " .. L"TXT_KEY_DIPLO_GOLD_PER_TURN" )
...
--		toolTip = toolTip .. L"TXT_KEY_DIPLO_OPEN_BORDERS_HAVE":gsub("%[COLOR_NEGATIVE_TEXT%]","[COLOR_POSITIVE_TEXT]")
	...
	elseif IsCiv5notVanilla and not g_pUsTeam:HasEmbassyAtTeam( g_iThemTeam ) then
...
--		toolTip = toolTip .. L"TXT_KEY_DIPLO_OPEN_BORDERS_THEY_HAVE":gsub("%[COLOR_NEGATIVE_TEXT%]","[COLOR_POSITIVE_TEXT]")
	if not g_pUsTeam:IsOpenBordersTradingAllowed() then
		toolTip = toolTip .. "[COLOR_WARNING_TEXT]" .. L(IsCiv5notVanilla and "TXT_KEY_DIPLO_OPEN_BORDERS_NO_TECH" or "TXT_KEY_DIPLO_NEED_WRITING_TT") .. "[ENDCOLOR]"
	elseif IsCiv5notVanilla and not g_pThemTeam:HasEmbassyAtTeam( g_iUsTeam ) then
...
--		toolTip = toolTip .. "[COLOR_POSITIVE_TEXT]" .. L"TXT_KEY_DIPLO_DEF_PACT_EXISTS" .. "[ENDCOLOR]"
	...
	elseif IsCiv5notVanilla and ( not g_pUsTeam:HasEmbassyAtTeam( g_iThemTeam ) or not g_pThemTeam:HasEmbassyAtTeam( g_iUsTeam ) ) then
...
--		toolTip = L"TXT_KEY_DIPLO_TRADE_AGREEMENT_NO_AGREEMENT"
	...
	for resourceID, instance in pairs( g_UsPocketResources ) do
		...
		if g_Deal:IsPossibleToTradeItem( g_iUs, g_iThem, TradeableItems.TRADE_ITEM_RESOURCES, resourceID, 1 ) then	-- 1 here is 1 quanity of the Resource, which is the minimum possible
			...
			instance.Button:SetText( resource.IconString .. resource._Name .. " (" .. g_pUs:GetNumResourceAvailable( resourceID, false ) .. ")" )
		else
	...
	for resourceID, instance in pairs( g_ThemPocketResources ) do
		if g_Deal:IsPossibleToTradeItem( g_iThem, g_iUs, TradeableItems.TRADE_ITEM_RESOURCES, resourceID, 1 ) then -- 1 here is 1 quanity of the Resource, which is the minimum possible
			...
			instance.Button:SetText( resource.IconString .. " " .. resource._Name .. " (" .. g_pThem:GetNumResourceAvailable( resourceID, false ) .. ")" )
		else
			instance.Button:SetHide( true )
		end
	end
...
function OnBack( flag )
	...
	if g_bTradeReview or g_PVPTrade then
		...
		elseif g_diploUIStateID == DiploUIStateTypes.DIPLO_UI_STATE_TRADE_AI_MAKES_REQUEST then
			...
			DoClearDeal()
			DisplayDeal()
			Game.DoFromUIDiploEvent( FromUIDiploEventTypes.FROM_UI_DIPLO_EVENT_REQUEST_HUMAN_REFUSAL, g_iThem, IsCiv5notVanilla and g_Deal and g_pUs:GetDealMyValue(g_Deal) or 0, 0 )
...
function()
	...
	if g_PVPTrade then
		...
		elseif g_PVPTrade == 1 then -- withdraw our proposal
			...
			if g_Deal:GetNumItems() == 0 then
				if IsCiv5BNW and g_pUsTeam:CanChangeWarPeace( g_iThemTeam ) and not g_pUsTeam:IsAtWar( g_iThemTeam ) and g_pUsTeam:CanDeclareWar( g_iThemTeam ) then
...
-----------------------------------------------------------------------------------------------------------------------
do
	local function AddGoldPerTurnTrade( control, playerID, player, goldPerTurn )
		g_Deal:AddGoldPerTurnTrade( playerID, min( UI.ShiftKeyDown() and huge or goldPerTurn, player:CalculateGoldRate() ), g_iDealDuration )
		DoUIDealChangedByHuman()
		control:TakeFocus()
	end
	Controls.UsPocketGoldPerTurn:RegisterCallback( Mouse.eLClick, function() AddGoldPerTurnTrade( Controls.UsGoldPerTurnAmount, g_iUs, g_pUs, 1 ) end )
	Controls.ThemPocketGoldPerTurn:RegisterCallback( Mouse.eLClick, function() AddGoldPerTurnTrade( Controls.ThemGoldPerTurnAmount, g_iThem, g_pThem, 2 ) end )
...
do
	local function AddResourceTrade( playerID, resourceID )
		g_Deal:AddResourceTrade( playerID, resourceID, min( Players[playerID]:GetNumResourceAvailable( resourceID, false ), Game.GetResourceUsageType(resourceID) == ResourceUsageTypes.RESOURCEUSAGE_STRATEGIC and UI.ShiftKeyDown() and huge or 1 ), g_iDealDuration )
		DoUIDealChangedByHuman()
	end
	...
	local function AddResourceInstance( resource, instanceName, stack, list, clickAction, changeAction )
		...
		local button = instance.Button
		button:SetText( resource.IconString .. resource._Name )
		button:SetVoid1( resource.ID )
		button:RegisterCallback( Mouse.eLClick, clickAction )
		button:SetToolTipCallback( LuaEvents.ResourceToolTip.Call )
	...
	local function ChangeResourceAmount( string, control, playerID )
		...
		local resourceQuantity = min( Players[ playerID ]:GetNumResourceAvailable( resourceID, false ), tonumber(string) or 0 )
...
--print("init leader stack", stack:GetID(), onButtonClick )
		for playerID = 0, GameDefines.MAX_CIV_PLAYERS-1 do
			...
			if player and player:IsEverAlive() then
				...
				leaderName = leaderName .. " (" .. GameInfo.Civilizations[player:GetCivilizationType()]._Name .. ")"
