...
function OnHostConnect()
	Controls.JoiningLabel:LocalizeAndSetText( "TXT_KEY_MULTIPLAYER_JOINING_PLAYERS" )
end
...
function ShowHideHandler( bIsHide, bIsInit )
	if not bIsHide then
		...
		if not g_joinFailed and not ContextPtr:IsHotLoad() then
			UIManager:SetUICursor( 1 )
			print( "Modding.ActivateAllowedDLC" )
