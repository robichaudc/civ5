...
----------------------------------------------------------------        
----------------------------------------------------------------        
Controls.SelectReplayButton:RegisterCallback( Mouse.eLClick, function()
	local replayFile = g_FileList[ g_iSelected ]
	if replayFile then
		UIManager:SetUICursor( 1 )
		Modding.ActivateModsAndDLCForReplay(replayFile)
		Events.SystemUpdateUI( SystemUpdateUIType.RestoreUI, "LoadReplay", replayFile )
	end
end)
...
----------------------------------------------------------------        
----------------------------------------------------------------        
local OnBack = Events.ExitToMainMenu.Call
