...
for resource in GameInfo.Resources() do
	if GetResourceUsageType(resource.ID) == RESOURCEUSAGE_LUXURY then
...
-- todo TradeableItems.TRADE_ITEM_THIRD_PARTY_WAR & permanent war
	...
	TheirTradeItems = function( playerID )
		local player = Players[ playerID ]
		if player then
			return L( "TXT_KEY_DIPLO_ITEMS_LABEL", player:GetCivilizationAdjective() )
		end
	end;

	OurTradeItems = function()
		return L"TXT_KEY_DIPLO_YOUR_ITEMS_LABEL", GetActivePlayer()
	end;
				...
				for playerID = 0, #Players do
					...
					then
						...
						if isMinorCiv then
							...
							n = ScratchDeal:IsPossibleToTradeItem( playerID, activePlayerID, TRADE_ITEM_RESOURCES, resourceID, 1 ) and player:GetNumResourceAvailable( resourceID, false )
						end
						if n and n > 0 then
							insert( civs, player:GetCivilizationShortDescription() .. " " .. n .. resource.IconString )
						end
					end
				end
				tip = L( "TXT_KEY_CITYVIEW_RESOURCE_DEMANDED", resource.IconString .. " " .. resource._Name ).."[NEWLINE]"..L"TXT_KEY_EO_RESOURCES_AVAILBLE"..": "
				if #civs > 0 then
					tip = tip .. concat(civs, ", " )
				else
					tip = tip .. L"TXT_KEY_MISC_UNKNOWN"
				end
			end
		end
		return tip
